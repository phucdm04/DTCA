...
trainer = Trainer(
    ...
    compute_metrics=build_compute_metrics_fn(text_inputs=data_inputs["dev"],pairs=dev_pairs),
)
trainer.train()

os.makedirs(output_dir, exist_ok=True)
model_file = os.path.join(output_dir, "final_model.pt")
torch.save(vb_model.state_dict(), model_file)
print(f"Final model saved to {model_file}")

trainer.compute_metrics = None
test_outputs = trainer.predict(test_dataset)
test_text_logits, test_cross_logits = test_outputs.predictions
test_pred_labels = np.argmax(test_cross_logits, -1)
test_text_pred_labels = np.argmax(test_text_logits, -1)
test_precision, test_recall, test_f1 = cal_f1(test_pred_labels, data_inputs["test"], test_pairs)
test_text_precision, test_text_recall, test_text_f1 = cal_f1(test_text_pred_labels, data_inputs["test"], test_pairs)
test_metric = {"precision": test_precision, "recall": test_recall, "f1": test_f1}
test_text_metric = {"precision": test_text_precision, "recall": test_text_recall, "f1": test_text_f1}
...
with open(output_result_file,"a",encoding="utf-8") as f:
    ...
    f.write("dev_multi_best: "+ str(best_metric)+"\n")
    f.write("dev_text_best: "+ str(text_best_metric)+"\n")
    f.write("test_multi: "+ str(test_metric)+"\n")
    f.write("test_text: "+ str(test_text_metric)+"\n")
    f.write("\n")
