...
set OUT_DIR=results\hotel_acsa_dtca_%TEXT_MODEL%_vit
set BATCH_SIZE=8
set EPOCHS=10
set LR=2e-5
...
if exist "%OUT_DIR%\best_model\pytorch_model.bin" (
    echo Found existing DTCA ACC/ACSA checkpoint: %OUT_DIR%\best_model
    echo Running evaluation only.
    python eval_acsa_dtca_hotel.py ^
        --data_dir %DATA_DIR% ^
        --image_dir %IMAGE_DIR% ^
    ...
    echo Training and evaluating DTCA ACC/ACSA model: %TEXT_MODEL% + ViT
    python train_acsa_dtca_hotel.py ^
        --data_dir %DATA_DIR% ^
        --image_dir %IMAGE_DIR% ^
        --text_model_name %TEXT_MODEL% ^
        --image_model_path %IMAGE_MODEL% ^
...
if errorlevel 1 (
    echo Hotel DTCA ACC/ACSA run failed.
    exit /b 1
)

echo.
echo Done. ACC/ACSA reports:
echo %OUT_DIR%\acc_category_report.txt
echo %OUT_DIR%\sentiment_report.txt
echo %OUT_DIR%\acsa_pred_vs_gold.tsv
echo %OUT_DIR%\acsa_summary_counts.json
endlocal
